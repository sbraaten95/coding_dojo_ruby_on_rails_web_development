...
RSpec.describe 'user profile page' do
  ...
  it "displays a user's secrets on profile page" do
    secret = @user.secrets.create(content: 'secret', user:@user)
    visit "/users/#{@user.id}"
    expect(page).to have_text("#{secret.content}")
  end
  it "displays everyone's secrets" do
    user2 = create_user 'julius', 'julius@lakers.com'
    secret1 = @user.secrets.create(content: 'secret', user:@user)
    secret2 = user2.secrets.create(content: 'secret', user:user2)
    visit '/secrets'
